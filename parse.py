...
try:
    for filename in os.listdir(input_dir):
        ...
        try:
            ...
            features = []
            for ul in soup.select("section.ClassifiedDetailPropertyGroups-group ul.ClassifiedDetailPropertyGroups-groupList"):
            ...
            get_error_logger().error(f"PARSE {filename} FAILED {duration}ms\n{traceback.format_exc()}\nHTML SNIPPET:\n{snippet}")
            exit_code = 3
            raise e

except FileNotFoundError as e:
    get_error_logger().error(f"Configuration error: {str(e)}\n{traceback.format_exc()}")
    exit_code = 1
except Exception as e:
    get_error_logger().error(f"Fatal error: {str(e)}\n{traceback.format_exc()}")
    if exit_code == 0:
        exit_code = 4
finally:
